...
class Helper
{
    ...
    public static function taxcalCulate($original_amount){
        $gettax = Setting::first();
        $taxRate = 0;
        if($gettax->services_tax==''){
            $taxRate = 0;
        }else{
            $taxRate = $gettax->services_tax;
        }

        $result = $original_amount * ($taxRate / 100);
