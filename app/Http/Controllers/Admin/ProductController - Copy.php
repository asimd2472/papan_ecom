...
class ProductController extends Controller
{
    ...
    public function product_list(Request $request){
        ...
        if($request->category_id!=''){
            $products = Product::with(['productImage', 'productCategory'])->where('category_id', $request->category_id)->orderBy('id', 'DESC')->paginate(30);
            // Session::put('key_product_id', $request->category_id);
        }
                ...
                if ($request->has('attribute') && is_array($request->attribute) && count($request->attribute) > 0) {
                    ...
                    foreach ($request->attribute as $key=>$attribute) {
                        ...
                        if (isset($attribute['attribute_name']) && $attribute['attribute_name'] !== '') {
                            ...
                            if(count($attribute['item']) > 0){
                                // ProductAttributeItem::where('product_id', $product_id)->delete();
                                // ProductAttributeItemSpecification::where('product_id', $product_id)->delete();
                                // ProductAttributeItemImage::where('product_id', $product_id)->delete();
    ...
    public function edit_productcategory($id){
        ...
        // dd($select_brand);
        return view('admin.product.edit_productcategory', compact('brand', 'productcategory', 'catrgory_brand', 'select_brand'));
    }
