...
<div class="bedcrumb-wrap row justify-content-between align-items-center">
    <div class="col-auto">
        <div class="page-name">Type</div>
    </div>
    <div class="col-auto">
        <div class="bedcrumb-list">
            <ul class="d-flex">
                <li><a href="{{url('admin/dashboard')}}">Type</a></li>
                <li>Type List</li>
            </ul>
        </div>
    </div>
...
<div class="content-wraper mt-3">
    <div class="content-wraper-inner">
        <a href="{{url('admin/create-type')}}" class="dark-btn-A">Create New</a>
    </div>
    <table class="table table-bordered">
        ...
        <tbody>
            ...
            @if(!empty($item->productCategory))
                <tr>
                    <td>{{$item->typename}}</td>
                    <td><img src="{{asset('storage/images/'.$item->typeimage)}}" width="60" alt=""></td>
                    <td>{{$item->productCategory->title}}</td>
                    <td>
                        <a href="{{url('admin/edit_type')}}/{{$item->id}}" class="btn btn-warning btn-sm"><i class="fa-solid fa-pen-to-square"></i></a>
                        <a href="{{url('admin/delete_type')}}/{{$item->id}}" onclick="return confirm('Are you sure you want to delete?');" class="btn btn-danger btn-sm"><i class="fa-sharp fa-solid fa-trash"></i></a>
                    </td>
                </tr>
            @endif
            @endforeach
      ...
      {{ $producttype->links('vendor.pagination.bootstrap-5') }}
</div>
@endsection
