                    ...
                    <div class="shopReelsType">
                        ...
                        <div class="row justify-content-center g-3">
                            @foreach ($products as $productsItem)
                                <div class="col-lg-4 col-md-4 col-sm-6 col-6 wow fadeInUpSort delay1">
                                    <a href="{{ route('product_details', ['product_slug' => $productsItem->product_slug])}}" class="shopBox">
                                        @if($productsItem->main_image_name!='')
                                            <img src="{{asset("upload/product/images/".$productsItem->main_image_name)}}" alt="{{$productsItem->product_title}}" />
                                        @else
                                            <img src="{{ Vite::asset('resources/front/images/no-image-available.jpg')}}" alt="no-image-available.jpg" />
                                        @endif
                                        <div class="shopBoxBtm text-start">
                                            <h6>New</h6>
                                            <h5>{{$productsItem->product_title}}</h5>
                                            @if ($productsItem->is_variation==0)
                                                <p><span><del>${{$productsItem->product_price}}</del></span> ${{$productsItem->product_offerprice}}</p>
                                            @else
                                                @php
                                                    $productAttributeItems = $productsItem->ProductAttributeItem;
